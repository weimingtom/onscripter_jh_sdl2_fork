    ...
    int trans_mode, Uint32 mask_value, SDL_Rect *clip, SDL_Surface *src1, SDL_Surface *src2, SDL_Surface *dst)
{
    ...
    if ( (trans_mode == ALPHA_BLEND_FADE_MASK ||
        ...
        struct Blender {
		public:  typedef ONScripter::ONSBuf ONSBuf;
		...
		blender.stsrc1_buffer = (ONSBuf *)src1->pixels + src1->w * rect.y + rect.x;
        blender.stsrc2_buffer = (ONSBuf *)src2->pixels + src2->w * rect.y + rect.x;
        blender.stdst_buffer = (ONSBuf *)dst->pixels + dst->w * rect.y + rect.x;
        blender.screen_width = screen_width;
		blender.mask_surface = mask_surface;
		blender.rect = &rect;
		blender.mask_value = mask_value;
		blender.lowest_mask = lowest_mask;
		blender.overflow_mask = overflow_mask;
